...
func TestResharing_minoch(t *testing.T) {
	...
	bdnPubkeysOld := make([]kyber.Point, len(minosOld))
	...
	for i, m := range minosOld {
		pdkg, pubkey, bdnPubkey := NewPedersen(m)
		dkgsOld[i] = pdkg
		pubkeysOld[i] = pubkey
		bdnPubkeysOld[i] = bdnPubkey
	}

	fakeAuthority := NewAuthority(addrsOld, pubkeysOld)
	fakeAuthorityBdn := NewAuthorityBls(addrsOld, bdnPubkeysOld)
	...
	_, err := actorsOld[1].Setup(fakeAuthority, fakeAuthorityBdn, thresholdOld)
	...
	// a pedersen
	for i := 0; i < nNew; i++ {
		pdkg, pubkey, _ := NewPedersen(minosNew[i+nCommon])
...
func TestResharing_minogrpc(t *testing.T) {
	...
	for i, mi := range minosOld {
		...
		pdkg, pubkey, bdnPubkey := NewPedersen(mi.(*minogrpc.Minogrpc))
	...
	for i, mi := range minosNew[nCommon:] {
		...
		pdkg, pubkey, _ := NewPedersen(mi.(*minogrpc.Minogrpc))
		dkgsNew[i+nCommon] = pdkg
		pubkeysNew[i+nCommon] = pubkey
	}
...
func TestResharingTwice(t *testing.T) {
	...
	bdnPubkeys1 := make([]kyber.Point, len(minos1))
	minoManager := minoch.NewManager()
	...
	for i, mi := range minos1 {
		pdkg, pubkey, bdnPubkey := NewPedersen(mi)
		dkgs1[i] = pdkg
		pubkeys1[i] = pubkey
		bdnPubkeys1[i] = bdnPubkey
	}

	fakeAuthority := NewAuthority(addrs1, pubkeys1)
	fakeAuthorityBdn := NewAuthorityBls(addrs1, bdnPubkeys1)
	...
	_, err := actors1[1].Setup(fakeAuthority, fakeAuthorityBdn, threshold1)
	require.NoError(t, err, initDkgFailed)
	...
	// a pedersen
	for i := 0; i < n2; i++ {
		pdkg, pubkey, _ := NewPedersen(minos2[i+nCommon12])
	...
	// a pedersen
	for i := 0; i < n3; i++ {
		pdkg, pubkey, _ := NewPedersen(minos3[i+2])
		dkgs3[i+2] = pdkg
		pubkeys3[i+2] = pubkey
	}
