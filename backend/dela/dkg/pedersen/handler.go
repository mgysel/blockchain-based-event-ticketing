...
func NewHandler(privKey kyber.Scalar, bdnPrivKey kyber.Scalar, bdnPubKey kyber.Point, me mino.Address) *Handler {
	log := dela.Logger.With().Str("role", "DKG handler").Str("addr", me.String()).Logger()

	return &Handler{
		log: log,

		dkgInstance: newInstance(log, me, privKey, bdnPrivKey, bdnPubKey),
	}
}
