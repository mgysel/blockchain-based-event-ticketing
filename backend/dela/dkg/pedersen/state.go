...
func (s *state) init(participants []mino.Address, pubkeys []kyber.Point, bdnPubKeys []kyber.Point, masks []*sign.Mask, t int) {
	s.Lock()
	defer s.Unlock()

	s.participants = participants
	s.pubkeys = pubkeys
	s.bdnPubkeys = bdnPubKeys
	s.masks = masks
	s.threshold = t
}
...
func (s *state) getBdnPublicKeys() []kyber.Point {
	s.Lock()
	defer s.Unlock()
	return s.bdnPubkeys
}

func (s *state) getMasks() []*sign.Mask {
	s.Lock()
	defer s.Unlock()
	return s.masks
}
