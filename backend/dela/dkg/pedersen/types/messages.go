...
// SignRequest is the message the initiator of the Cosi protocol should send to all the
// nodes.
//
...
func NewSignRequest(message []byte, index int) SignRequest {
	return SignRequest{
		message: message,
		index:   index,
	}
...
// GetMessage returns the message to be signed.
func (s SignRequest) GetMessage() []byte {
	return s.message
}

// GetIndex returns the index of the message to be signed.
func (s SignRequest) GetIndex() int {
	return s.index
}

// Serialize implements serde.Message. It looks up the format and returns the
// serialized data for the signRequest message.
func (s SignRequest) Serialize(ctx serde.Context) ([]byte, error) {
...
// SignResponse is the message the nodes int the Cosi protocol should send back to
// the initiator node.
//
// - implements serde.Message
...
func NewSignResponse(signature []byte, index int, pk kyber.Point) SignResponse {
	return SignResponse{
		signature: signature,
		index:     index,
		publicKey: pk,
...
func (s SignResponse) GetSignature() []byte {
	return s.signature
}

// GetIndex returns the index of the signer.
func (s SignResponse) GetIndex() int {
	return s.index
}

// GetPublicKey returns the index of the signer.
func (s SignResponse) GetPublicKey() kyber.Point {
	return s.publicKey
}

// Serialize implements serde.Message. It looks up the format and returns the
// serialized data for the signResponse message.
func (s SignResponse) Serialize(ctx serde.Context) ([]byte, error) {
	format := msgFormats.Get(ctx.GetFormat())
...
type Start struct {
	...
	pubkeys []kyber.Point
	// the corresponding kyber.Point bdn pub keys of the addresses
	bdnPubkeys []kyber.Point
}

// NewStart creates a new start message.
func NewStart(thres int, addrs []mino.Address, pubkeys []kyber.Point, bdnPubkeys []kyber.Point) Start {
	return Start{
		thres:      thres,
		addresses:  addrs,
		pubkeys:    pubkeys,
		bdnPubkeys: bdnPubkeys,
	}
}
...
// GetBdnPublicKeys returns the list of public keys.
func (s Start) GetBdnPublicKeys() []kyber.Point {
	return emptyIfNil(s.bdnPubkeys)
}
