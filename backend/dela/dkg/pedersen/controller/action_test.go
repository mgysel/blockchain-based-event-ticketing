...
func TestGetCollectiveAuth(t *testing.T) {
	_, pk := bdn.NewKeyPair(pairing.NewSuiteBn256(), suite.RandomStream())
	pkbuf, err := pk.MarshalBinary()
	require.NoError(t, err)
	pubKey := fmt.Sprintf("RjEyNy4wLjAuMToyMDAx:RcTqbSXCIkZmmaGjLVAZs8TdTvq7b3SFr14F89h6ID8=:%s", base64.StdEncoding.EncodeToString(pkbuf))
...
func TestEncodeAuthority_marshalFail(t *testing.T) {
	...
	_, err := encodeAuthority(ctx, badPoint{}, badPoint{})
...
func TestEncodeAuthority_pkFail(t *testing.T) {
	...
	_, err := encodeAuthority(ctx, pk, bdnPk)
...
func TestDecodeAuthority_noMino(t *testing.T) {
	...
	_, _, _, err = decodeAuthority(ctx, pubKey)
...
func TestDecodeAuthority_noBase64Addr(t *testing.T) {
	pubKey := "aa:RcTqbSXCIkZmmaGjLVAZs8TdTvq7b3SFr14F89h6ID8=:RcTqbSXCIkZmmaGjLVAZs8TdTvq7b3SFr14F89h6ID8="
...
func TestDecodeAuthority_noBase64Pubkey(t *testing.T) {
	pubKey := "RjEyNy4wLjAuMToyMDAx:aa:RjEyNy4wLjAuMToyMDAx"
...
func TestDecodeAuthority_badUnmarshalPubkey(t *testing.T) {
	pubKey := "RjEyNy4wLjAuMToyMDAx:aaa=:aaa="
...
func TestIssueMasterCredentialAction_OK(t *testing.T) {
	a := issueMasterCredentialAction{}

	idHash := "idHash"
	masterCredential := []byte("Master Credential")
	masterSignatures := [][]byte{[]byte("Sig1"), []byte("Sig2")}
	...
	require.Equal(t, "TWFzdGVyIENyZWRlbnRpYWw=:U2lnMQ==:U2lnMg==", out.String())
}

func TestIssueEventCredentialAction_OK(t *testing.T) {
	a := issueEventCredentialAction{}
	...
	eventCredential := []byte("Master Credential")
	eventSignatures := [][]byte{[]byte("Sig1"), []byte("Sig2")}
...
func TestVerifyEventCredentialAction_OK(t *testing.T) {
	a := verifyEventCredentialAction{}

	eventCredential := []byte("Event Credential")
	verified := true
...
func TestEncryptAction_encodeFail(t *testing.T) {
	...
	err := a.Execute(ctx)
	require.EqualError(t, err, fake.Err("failed to encode encrypted: failed to marshal k"))
}
...
func TestEncryptAction_OK(t *testing.T) {
	...
	require.Equal(t, "ENCRYPT;success;"+dataHex+":"+dataHex+":", out.String())
}
...
func TestDecryptAction_decryptOK(t *testing.T) {
	...
	message := "fake"
	expected := "DECRYPT;success;" + hex.EncodeToString([]byte(message))
...
type fakeActor struct {
	...
	setupErr                 error
	issueMasterCredentialErr error
	issueEventCredentialErr  error
	verifyEventCredentialErr error
	encryptErr               error
	decryptErr               error
	vencryptErr              error
	vdecryptErr              error
	...
	idHash           string
	masterCredential []byte
	masterSignatures [][]byte
	eventCredential  []byte
...
func (f fakeActor) Setup(co crypto.CollectiveAuthority, coBdn crypto.CollectiveAuthority, threshold int) (pubKey kyber.Point, err error) {
	return suite.Point(), f.setupErr
}

func (f fakeActor) IssueMasterCredential(idHash string) (masterCredential []byte, masterSignatures [][]byte, err error) {
	return f.masterCredential, f.masterSignatures, f.issueMasterCredentialErr
}

func (f fakeActor) IssueEventCredential(idHash string, eventName string, masterCredential []byte, masterSignatures [][]byte) (eventCredential []byte, eventSignatures [][]byte, err error) {
	return f.eventCredential, f.eventSignatures, f.issueEventCredentialErr
}

func (f fakeActor) VerifyEventCredential(idHash string, eventName string, eventCredential []byte, eventSignatures [][]byte) (verified bool, err error) {
	return f.verified, f.verifyEventCredentialErr
}
