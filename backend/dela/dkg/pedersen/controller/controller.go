...
func (m minimal) SetCommands(builder node.Builder) {
	...
	sub.SetFlags(
		cli.StringSliceFlag{
			Name:  "authority",
			Usage: "<ADDR>:<PK>:<PK> string, where each token is encoded in base64",
		},
		cli.IntFlag{
			Name:  "threshold",
	...
	sub = cmd.SetSubCommand("issueMasterCredential")
	sub.SetDescription("issues a master credential")
	sub.SetFlags(
		cli.StringFlag{
			Name:  "idhash",
			Usage: "the mpc hash of the identification",
		},
	)
	sub.SetAction(builder.MakeAction(issueMasterCredentialAction{}))

	sub = cmd.SetSubCommand("issueEventCredential")
	sub.SetDescription("issues an event credential")
	sub.SetFlags(
		cli.StringFlag{
			Name:  "idhash",
			Usage: "the mpc hash of the identification",
		...
		cli.StringFlag{
			...
			Usage: "the master credential issued",
		},
		cli.StringFlag{
			Name:  "masterSignatures",
			Usage: "the master signatures issued",
		},
	)
	sub.SetAction(builder.MakeAction(issueEventCredentialAction{}))

	sub = cmd.SetSubCommand("verifyEventCredential")
	sub.SetDescription("verifies an event credential")
	sub.SetFlags(
		cli.StringFlag{
			Name:  "idhash",
			Usage: "the mpc hash of the identification",
		...
		cli.StringFlag{
			...
			Usage: "the event credential issued",
		},
		cli.StringFlag{
			Name:  "eventSignatures",
			Usage: "the event signatures issued",
		},
	)
	sub.SetAction(builder.MakeAction(verifyEventCredentialAction{}))
...
func (m minimal) OnStart(ctx cli.Flags, inj node.Injector) error {
	...
	dkg, pubkey, bdnPubkey := pedersen.NewPedersen(no)
	...
	bdnPubkeyBuf, err := bdnPubkey.MarshalBinary()
	if err != nil {
		return xerrors.Errorf("failed to encode pubkey: %v", err)
	}
