...
type Message struct {
	...
	SignResponse             *SignResponse             `json:",omitempty"`
...
func (f msgFormat) Encode(ctx serde.Context, msg serde.Message) ([]byte, error) {
	...
	case types.SignResponse:
		m, err = encodeSignResponse(in)
	case types.Start:
...
func (f msgFormat) Decode(ctx serde.Context, data []byte) (serde.Message, error) {
	...
	case m.SignResponse != nil:
		return f.decodeSignResponse(ctx, m.SignResponse)
...
func encodeSignRequest(msg types.SignRequest) (Message, error) {
	...
	return Message{SignRequest: &signRequest}, nil
}

func (f msgFormat) decodeSignRequest(ctx serde.Context,
	signRequest *SignRequest) (serde.Message, error) {

	return types.NewSignRequest(signRequest.Message, signRequest.Index), nil
}

func encodeSignResponse(msg types.SignResponse) (Message, error) {
	if msg.GetPublicKey() == nil {
		return Message{}, xerrors.Errorf("public key is nil")
	}

	data, err := msg.GetPublicKey().MarshalBinary()
	if err != nil {
	...
	return Message{SignResponse: &signResponse}, nil
}

func (f msgFormat) decodeSignResponse(ctx serde.Context,
	signResponse *SignResponse) (serde.Message, error) {

	var point kyber.Point = pairing.NewSuiteBn256().Point()
	err := point.UnmarshalBinary(signResponse.PublicKey)
	if err != nil {
		return nil, xerrors.Errorf("couldn't unmarshal public key: %v", err)
	}

	return types.NewSignResponse(signResponse.Signature, signResponse.Index, point), nil
}
...
func encodeStart(msg types.Start) (Message, error) {
	...
	bdnPubkeys := make([]PublicKey, len(msg.GetBdnPublicKeys()))
	for i, bdnPubkey := range msg.GetBdnPublicKeys() {
		data, err := bdnPubkey.MarshalBinary()
		if err != nil {
			return Message{}, xerrors.Errorf("couldn't marshal bdn public key: %v", err)
		}
...
func (f msgFormat) decodeStart(ctx serde.Context, start *Start) (serde.Message, error) {
	...
	bdnPubkeys := make([]kyber.Point, len(start.BdnPublicKeys))
	for i, bdnPubkey := range start.BdnPublicKeys {
		point := pairing.NewSuiteBn256().Point()
		err := point.UnmarshalBinary(bdnPubkey)
		if err != nil {
			return nil, xerrors.Errorf("couldn't unmarshal public key: %v", err)
		}
	...
	s := types.NewStart(start.Threshold, addrs, pubkeys, bdnPubkeys)
