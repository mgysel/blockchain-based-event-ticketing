...
func TestMessageFormat_SignRequest_Encode(t *testing.T) {
	msg := []byte("Message")
	index := 0
	signRequest := types.NewSignRequest(msg, index)

	format := newMsgFormat()
	ctx := serde.NewContext(fake.ContextEngine{})

	data, err := format.Encode(ctx, signRequest)
	require.NoError(t, err)
	regexp := `{"SignRequest":{"Message":"TWVzc2FnZQ==","Index":0}}`
	require.Regexp(t, regexp, string(data))

	_, err = format.Encode(fake.NewBadContext(), types.SignRequest{})
...
func TestMessageFormat_SignResponse_Encode(t *testing.T) {
	signature := []byte("Signature")
	index := 0
	_, pk := bdn.NewKeyPair(pairing.NewSuiteBn256(), suite.RandomStream())
	signResponse := types.NewSignResponse(signature, index, pk)

	format := newMsgFormat()
	ctx := serde.NewContext(fake.ContextEngine{})

	data, err := format.Encode(ctx, signResponse)
	require.NoError(t, err)
	regexp := `{"SignResponse":{"Signature":"U2lnbmF0dXJl","Index":0,"PublicKey":"[^"]+"}}`
	require.Regexp(t, regexp, string(data))

	_, err = format.Encode(fake.NewBadContext(), types.NewSignResponse(signature, index, pk))
	require.EqualError(t, err, fake.Err("couldn't marshal"))
...
func TestMessageFormat_Start_Encode(t *testing.T) {
	...
	start := types.NewStart(1, []mino.Address{fake.NewAddress(0)}, []kyber.Point{suite.Point()}, []kyber.Point{pairing.NewSuiteBn256().Point()})
	...
	require.NoError(t, err)
	regexp := `{"Start":{"Threshold":1,"Addresses":["AAAAAA=="],"PublicKeys":["AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA="],"BdnPublicKeys":["AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA="]}}`
	require.Equal(t, regexp, string(data))

	start = types.NewStart(0, []mino.Address{fake.NewBadAddress()}, nil, nil)
	_, err = format.Encode(ctx, start)
	require.EqualError(t, err, fake.Err("failed to encode message: couldn't marshal address"))

	start = types.NewStart(0, nil, []kyber.Point{badPoint{}}, nil)
	_, err = format.Encode(ctx, start)
...
func TestMessageFormat_Decode(t *testing.T) {
	...
	expected := types.NewStart(
		...
		[]kyber.Point{suite.Point()},
		[]kyber.Point{pairing.NewSuiteBn256().Point()},
	)
...
func TestMessageFormat_Decode_SignRequest(t *testing.T) {
	format := newMsgFormat()
	ctx := serde.NewContext(fake.ContextEngine{})
	ctx = serde.WithFactory(ctx, types.AddrKey{}, fake.AddressFactory{})
	...
	msg := []byte("Message")
	index := 0
	expected := types.NewSignRequest(msg, index)

	data, err := format.Encode(ctx, expected)
	require.NoError(t, err)

	signRequest, err := format.Decode(ctx, data)
	require.NoError(t, err)
	require.Equal(t, expected.GetMessage(), signRequest.(types.SignRequest).GetMessage())
}

func TestMessageFormat_Decode_SignResponse(t *testing.T) {
	format := newMsgFormat()
	ctx := serde.NewContext(fake.ContextEngine{})
	ctx = serde.WithFactory(ctx, types.AddrKey{}, fake.AddressFactory{})
	...
	index := 0
	_, pk := bdn.NewKeyPair(pairing.NewSuiteBn256(), suite.RandomStream())
	expected := types.NewSignResponse(signature, index, pk)

	data, err := format.Encode(ctx, expected)
	require.NoError(t, err)

	signResponse, err := format.Decode(ctx, data)
	require.NoError(t, err)
	require.Equal(t, expected.GetSignature(), signResponse.(types.SignResponse).GetSignature())
}
