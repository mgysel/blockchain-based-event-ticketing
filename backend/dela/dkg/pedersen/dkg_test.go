...
func TestDKGInstance_HandleSignRequestFail(t *testing.T) {
	out := &bytes.Buffer{}
	log := zerolog.New(out)
	...
	err := s.handleMessage(context.TODO(), types.SignRequest{}, fake.NewAddress(0), nil)
...
func TestDKGInstance_Start(t *testing.T) {
	...
	_, bdnPubKey := bdn.NewKeyPair(bdnSuite, suite.RandomStream())
	...
	start := types.NewStart(0, []mino.Address{fake.NewAddress(0)}, []kyber.Point{}, []kyber.Point{})
	...
	start = types.NewStart(2, []mino.Address{fake.NewAddress(0),
		fake.NewAddress(1)}, []kyber.Point{pubKey, suite.Point()}, []kyber.Point{bdnPubKey, bdnSuite.Point()})
