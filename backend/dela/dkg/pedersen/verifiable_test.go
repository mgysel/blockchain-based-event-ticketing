...
func Test_VerifiableEncDec_minoch(t *testing.T) {
	...
	for i, mino := range minos {
		dkg, pubkey, bdnPubkey := NewPedersen(mino)
		dkgs[i] = dkg
		pubkeys[i] = pubkey
	...
	fakeAuthorityBdn := NewAuthorityBls(addrs, bdnPubkeys)
	...
	_, err = actors[0].Setup(fakeAuthority, fakeAuthorityBdn, threshold)
	require.NoError(t, err)
...
func Test_VerifiableEncDec_minogrpc(t *testing.T) {
	...
	for i, mino := range minos {
		...
		dkg, pubkey, bdnPubkey := NewPedersen(mino.(*minogrpc.Minogrpc))
		dkgs[i] = dkg
		pubkeys[i] = pubkey
		bdnPubkeys[i] = bdnPubkey
	}
