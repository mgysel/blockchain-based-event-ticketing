...
func newInstance(log zerolog.Logger, me mino.Address, privKey kyber.Scalar, bdnPrivKey kyber.Scalar, bdnPubKey kyber.Point) *instance {
	return &instance{
		running:   true,
...
func (s *instance) handleMessage(ctx context.Context, msg serde.Message, from mino.Address, out mino.Sender) error {
	...
	case types.SignRequest:
		go func() {
			err := s.signRequest(ctx, msg, from, out)
			if err != nil {
...
// signRequest signs the message and sends the signature to the orchestrator.
func (s *instance) signRequest(ctx context.Context, msg types.SignRequest, from mino.Address, out mino.Sender) error {
	// Sign the message with the bdn private key
	if s.bdnPrivKey == nil {
		return xerrors.New("bdn private key is nil")
	}
	signature, err := bdn.Sign(pairing.NewSuiteBn256(), s.bdnPrivKey, msg.GetMessage())
	if err != nil {
		return xerrors.Errorf("failed to sign message: %v", err)
	}

	done := types.NewSignResponse(signature, msg.GetIndex(), s.bdnPubKey)

	select {
	case err = <-out.Send(done, from):
		if err != nil {
			return xerrors.Errorf("got an error while sending sign response: %v", err)
		}
	case <-ctx.Done():
...
func (s *instance) start(ctx context.Context, start types.Start, deals channel.Timed[types.Deal],
	...
	// Create masks for bdn public keys
	bdnPubkeys := start.GetBdnPublicKeys()
	masks := make([]*sign.Mask, len(bdnPubkeys))
	for i := 0; i < len(bdnPubkeys); i++ {
		masks[i], _ = sign.NewMask(bdnSuite, bdnPubkeys, nil)
	}

	s.startRes.init(start.GetAddresses(), start.GetPublicKeys(), start.GetBdnPublicKeys(), masks, start.GetThreshold())
...
func (s *instance) doReshare(ctx context.Context, start types.StartResharing,
	...
	case commonNode:
		...
		bdnPublicKeys := s.startRes.getBdnPublicKeys()
		masks := s.startRes.getMasks()
		s.startRes.init(start.GetAddrsNew(), start.GetPubkeysNew(), bdnPublicKeys, masks, start.GetTNew())
