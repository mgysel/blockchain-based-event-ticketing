...
func (ns *Service) Execute(snap store.Snapshot, step execution.Step) (execution.Result, error) {
	...
	dela.Logger.Info().Str("contract", name).Msgf("event contract set")
	dela.Logger.Info().Str("NS.contracts", name).Msgf("event contract set")

	contract := ns.contracts[name]
	if contract == nil {
		return execution.Result{}, xerrors.Errorf("unknown contract '%s'. Contracts: %s", name, ns.contracts)
	}
