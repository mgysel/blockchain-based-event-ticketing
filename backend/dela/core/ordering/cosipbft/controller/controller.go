...
import (
	...
	"go.dedis.ch/dela/contracts/bank"
...
func (m miniController) OnStart(flags cli.Flags, inj node.Injector) error {
	...
	event.RegisterContract(exec, event.NewContract(eventAccessKey[:], access))
	bank.RegisterContract(exec, bank.NewContract(valueAccessKey[:], access))
	auction.RegisterContract(exec, auction.NewContract(valueAccessKey[:], access))
	auctionF3B.RegisterContract(exec, auctionF3B.NewContract(valueAccessKey[:], access))
