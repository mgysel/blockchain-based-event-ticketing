...
// aKey is the access key used for the bank contract
var aKey = [32]byte{2}

// miniController is a CLI initializer to register the bank contract
//
// - implements node.Initializer
type miniController struct {
}

// NewController creates a new minimal controller for the bank contract.
...
func (m miniController) OnStart(flags cli.Flags, inj node.Injector) error {
	...
	contract := bank.NewContract(aKey[:], access)
	bank.RegisterContract(exec, contract)
