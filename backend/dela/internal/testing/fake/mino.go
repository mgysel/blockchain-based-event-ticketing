...
func NewPublicKeyIterator(signers []crypto.Signer) *PublicKeyIterator {
	fmt.Println("*** Inside mino NewPublicKeyIterator")
...
func (i *PublicKeyIterator) HasNext() bool {
	...
	fmt.Println("i.signers: ", i.signers)
	fmt.Println("len i.signers: ", len(i.signers))
...
func (i *PublicKeyIterator) GetNext() crypto.PublicKey {
	fmt.Println("*** Inside GetNext()")
	if i.HasNext() {
		fmt.Println("I has next")
		res := i.signers[i.index]
		i.index++
		fmt.Println("Res: ", res)
		fmt.Println("Res.GetPublicKey(): ", res.GetPublicKey())
...
// NewAuthorityBls returns a new collective authority of n members with new signers
func NewAuthorityBls(n int) CollectiveAuthority {
	signers := make([]crypto.Signer, n)
	for i := range signers {
