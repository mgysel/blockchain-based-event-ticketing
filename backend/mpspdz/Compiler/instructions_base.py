...
class Instruction(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not kwargs.get('copying', False):
            self.check_args()
        if kwargs.get('add_to_prog', True) and hasattr(program, 'curr_block'):
            # NOTE: MJG ADDED hasattr for curr_block
            program.curr_block.instructions.append(self)
        if hasattr(program, 'DEBUG'):
            # NOTE: MJG ADDED hasattr for DEBUG
            if program.DEBUG:
