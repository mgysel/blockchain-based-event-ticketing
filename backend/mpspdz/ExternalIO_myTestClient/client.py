...
import struct
import time
from Compiler.library import print_ln, for_range, vectorize
...
class Client:
    ...
    def receive_triples(self, T, n):
        ...
        for socket in self.sockets:
            ...
            print("OS LENGTH: ", os.get_length())
            print("n_expected: ", n_expected)
            print("T.size(): ", T.size())
            print("n: ", n)
            print("multiplication: ", n_expected * T.size() * n)
...
class octetStream:
    ...
    def Receive(self, socket):
        ...
        print("SOCKET: ", socket)
        recv = socket.recv(4)
        print("SOCKET RECEIVE: ", recv)
        print("LENGTH RECEIVE: ", len(recv))
        if len(recv) == 4:
            length = struct.unpack('<I', recv)[0]
            print("LENGTH: ", length)
            self.buf = b''
            while len(self.buf) < length:
                this_recv = socket.recv(length - len(self.buf))
                print("this RECEIVE: ", this_recv)
                self.buf += this_recv
